<script lang="ts">
  ...
  const totalHeight = $derived(genreRank.length * (yBarheight + 2));
  ...
  <svg {width} {height}>
    <g class="bars">
      {#each genreRank as [genre, cnt] (genre)}
        <g
          class={`${genre} genre`}
          transform={`translate(${usableArea.left}, ${yScale(genre) ?? 0})`}
        ...
        >
          ...
          <rect
            width={xScale(cnt) - usableArea.left}
            height={yBarheight}
          ...
          <text
            x={xScale(cnt) - usableArea.left + 5}
            y={(yBarheight + 2) / 2}
            font-size="12"
            text-anchor="start"
            alignment-baseline="middle"
