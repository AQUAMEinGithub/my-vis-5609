<script lang="ts">
    ...
    onMount(
        ...
        async () => {
            ...
            const rows = await d3.csv<Row>("./summer_movies.csv");
            const counts = new Map<string, number>();

            rows.forEach((d) => {
                const genres = d.genres ?? "";
                const main = genres.split(",")[0]?.trim();
                const key = main && main.length > 0 ? main : "Unknown";
                counts.set(key, (counts.get(key) ?? 0) + 1);
            });

            dummyData = Object.fromEntries(counts);
    ...
    function createBars(scene: THREE.Scene, font: Font, dummyData: TDummyData) {
        ...
        const barMaxWidth = 1000; // Maximum width of all bars
